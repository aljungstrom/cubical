...
open import Cubical.Data.Nat using (ℕ ; suc ; zero ; isSetℕ ; max ; min ; snotz)
...
postulate -- I am too lazy to prove this right now
  insertSorted-presSorted : (x : ℕ) (ys : List ℕ) → isSorted ys → isSorted (insertSorted x ys)
...
check2 : isTree (Node 1 (just (Node 0 nothing {!!})) {!!})
check2 = {!!}
