...
module Cubical.ZCohomology.FreudenthalFix.Code where
...
{-
baseCase : ∀{ℓ} {A : Type ℓ} (a : A) (n : ℕ) → (iscon : is- (ℕ→ℕ₋₂ n) -ConnectedType A) → (q : north ≡ south) → isEquiv (RlFun2 a a n iscon q)
...
code' {A = A} {a = a} n iscon north p = (∥ fiber (λ y → σ y {a}) p ∥ (ℕ→ℕ₋₂ (n + n)))
code' {ℓ} {A = A} {a = a} n iscon south q = ∥ fiber merid q ∥ (ℕ→ℕ₋₂ (n + n))
code' {ℓ} {A = A} {a = a} n iscon (merid c i) = toPathP {A = λ i → north ≡ merid c i → Type ℓ} {x = λ p → (∥ fiber (λ y → σ y {a}) p ∥ (ℕ→ℕ₋₂ (n + n)))} {y = λ q → ∥ fiber merid q ∥ (ℕ→ℕ₋₂ (n + n))} (transport (λ i → Lemma296 {x = north} {y = south} {A = λ x → north ≡ x} {B = λ _ → Type ℓ} ((merid c)) ( (λ p → (∥ fiber (λ y → σ y {a}) p ∥ (ℕ→ℕ₋₂ (n + n))))) (((λ q → ∥ fiber merid q ∥ (ℕ→ℕ₋₂ (n + n))))) (~ i)) (transport ({!funExt ?!}) λ (q : north ≡ south) → (ua ((RlFun2' a c n iscon q) , RlFunEquiv' a c n iscon q)))) i
  where
  helper : ((q : north ≡ south) → (∥ fiber (λ y → σ y {a}) (q ∙ sym (merid c)) ∥ ℕ→ℕ₋₂ (n + n)) ≡ (∥ fiber merid q ∥ ℕ→ℕ₋₂ (n + n))) ≡ Lemma296 {A = λ x → north ≡ x} {B = λ _ → Type ℓ} (merid c) (λ p → ∥ fiber (λ y → σ y {a}) p ∥ ℕ→ℕ₋₂ (n + n)) (λ q → ∥ fiber merid q ∥ ℕ→ℕ₋₂ (n + n)) i1
  helper = {!!}
