...
module Cubical.ZCohomology.FreudenthalCleanup.FinishingUp4  where


open import Cubical.ZCohomology.Base
open import Cubical.ZCohomology.FreudenthalCleanup.7-5-14
open import Cubical.ZCohomology.FreudenthalCleanup.Prelim
open import Cubical.ZCohomology.FreudenthalCleanup.Code
...
cId : (n : ℕ) (a x1 : A) (iscon : is- (ℕ→ℕ₋₂ n) -ConnectedType A) →
       ...
       ≡ transport (λ i → uncurry (CODE a n iscon) (pair⁼ (λ i₁ → merid a (~ i₁)) (pairLemma3 (merid x1) (λ i₁ → merid a (~ i₁))) i))
...
wowie {ℓ} {A = A} n a x1 iscon = wowie2 (south) (merid x1) 
  where
  wowie2 : (y : Susp A) → (p : north ≡ y) → (transport (λ i → uncurry (CODE a n iscon) (pair⁼ p (transpRCancel (λ i₁ → north ≡ p i₁) p) i))) ∣ a , rCancel (merid a) ∙ sym (rCancel p) ∙ sym (pairLemma3 p (sym p)) ∣
           ≡ transport (λ i → (uncurry (CODE a n iscon) (pair⁼ p (pairLemma2 p) i))) ∣ a , rCancel (merid a) ∣
  wowie2 y = J (λ y p → (transport (λ i → uncurry (CODE a n iscon) (pair⁼ p (transpRCancel (λ i₁ → north ≡ p i₁) p) i)))
    ...
    stupidAgda : transport (λ i → uncurry (CODE a n iscon) (pair⁼ (λ _ → north) (transportRefl (transport (λ _ → _≡_ {A = Susp A} north north) (λ _ → north)) ∙ transportRefl (λ _ → north)) i))
               ...
               ≡ transport (λ i → uncurry (CODE a n iscon)
                                          (north , (sym (transportRefl ((transport (λ z → _≡_ {A = Susp A} north north) (λ _ → north)))) ∙
                                                         ...
                                                         transportRefl (λ _ → north)) i))
                           ∣ a , rCancel (merid a) ∙ (λ i → rCancel (λ _ → north) (~ i)) ∙ (λ i → pairLemma3ReflRefl i1 (~ i)) ∣ 
    stupidAgda j = transport (λ i → uncurry (CODE a n iscon)
...
codeTranspHelper1 :  ∀{ℓ} {X : Type ℓ} {a b : X}  (q p : a ≡ b)
...
codeTranspHelper1 {ℓ}  {X = X} {a = a} = J (λ b q → (p : a ≡ b) (A : (a ≡ a) → Type ℓ) (B : (a ≡ b) → Type ℓ) (f : (q₁ : a ≡ b) → A (q₁ ∙ sym q) ≡ B q₁) →
                              ...
                              λ p A B f → 
                                         (λ k → ((λ i → (Lemma294Refl {A = λ x → a ≡ x} {B = λ _ → Type ℓ} A k) (~ i) p)) ∙
                                          ...
                                          (λ k i → ((cong (λ x → x (equivTest {X = X} (λ _ → a) f)) (Lemma296-funRefl {A = λ y → a ≡ y}
...
{- We will be able to reduce the inner transport to (RlFun2 a x1 n iscon (merid x1)) ...... 
...
test : (n : ℕ) (a x1 : A) (iscon : is- (ℕ→ℕ₋₂ n) -ConnectedType A) →
       (RlFun2 a x1 n iscon (merid x1)) (transport (λ i → cong (λ p₁ → ∥ fiber (λ y₁ → σ a y₁) p₁ ∥ ℕ→ℕ₋₂ (n + n)) (pairLemma3 (merid x1) (sym (merid x1))) i)
                                                  (transport (λ i → (transportRefl (∥ fiber (λ y₁ → σ a y₁)
                                                                    (subst (_≡_ north) (sym (merid x1)) (merid x1)) ∥ ℕ→ℕ₋₂ (n + n)))i)
                                                   ∣ a , rCancel (merid a) ∙ sym (rCancel (merid x1)) ∙ sym (pairLemma3 (merid x1) (sym (merid x1))) ∣))
        ≡ ∣ x1 , refl ∣
test n a x1 iscon = (RlFun2 a x1 n iscon (merid x1)) (outer (inner guy))
                                 ≡⟨ (λ j → (RlFun2 a x1 n iscon (merid x1)) (outer (innerTransp j))) ⟩
                    (RlFun2 a x1 n iscon (merid x1)) (outer guy)
                                 ≡⟨ (λ j → (RlFun2 a x1 n iscon (merid x1)) (outerTransp j)) ⟩
                    (RlFun2 a x1 n iscon (merid x1)) guy2 ≡⟨ refl ⟩
                    sufMap2 n x1 a a iscon (merid x1) (rCancel (merid a) ∙ sym (rCancel (merid x1)))
  ...
  outer = transport (λ i → cong (λ p₁ → ∥ fiber (λ y₁ → σ a y₁) p₁ ∥ ℕ→ℕ₋₂ (n + n)) (pairLemma3 (merid x1) (sym (merid x1))) i)
  ...
  outerTransp = (λ j →  transport (λ i → cong (λ p₁ → ∥ fiber (λ y₁ → σ a y₁) p₁ ∥ ℕ→ℕ₋₂ (n + n)) (pairLemma3 (merid x1) (sym (merid x1))) i)
                                  ∣ a , rUnit (rCancel (merid a) ∙ sym (rCancel (merid x1)) ∙ sym (pairLemma3 (merid x1) (sym (merid x1)))) j  ∣  ) ∙
                (λ j → transport (λ i → cong (λ p₁ → ∥ fiber (λ y₁ → σ a y₁) p₁ ∥ ℕ→ℕ₋₂ (n + n)) (pairLemma3 (merid x1) (sym (merid x1))) (i ∨ j))
                       ∣ a , (rCancel (merid a) ∙
...
outerTranspId : (n : ℕ) (a x1 : A) (iscon : is- (ℕ→ℕ₋₂ n) -ConnectedType A) →
                transport (λ i → uncurry (CODE a n iscon) (pair⁼ (sym (merid a)) (pairLemma3 (merid x1) (sym (merid a)) ) i)) ∣ x1 , refl ∣
                ≡ transport (λ i → uncurry (CODE a n iscon) (pair⁼ (λ i₁ → merid a (~ i₁)) (transpRCancel (λ i → north ≡ (merid a (~ i))) (merid x1 ∙ (sym (merid a)))) i)) ∣ x1 , rUnit (merid x1) ∙ sym (cong (λ x → merid x1 ∙ x) (lCancel (merid a)))  ∙ assocJ (merid x1) (sym (merid a)) (merid a) ∙ sym (pairLemma3 {a1 = north} (merid x1 ∙ (sym (merid a))) (merid a)) ∣
outerTranspId {ℓ} {A = A} n a x1 iscon = sym (wowie' north (sym (merid a)))
   where
   wowie' : (y : Susp A) → (q : south ≡ y) → transport (λ i → uncurry (CODE a n iscon) (pair⁼ q (transpRCancel (λ i → north ≡ q i) ((merid x1) ∙ q)) i))
                                                      ∣ x1 , rUnit (merid x1)  ∙
                                                             sym (cong (λ x → merid x1 ∙ x) (lCancel (sym q))) ∙
                                                             assocJ (merid x1) q (sym q) ∙ sym ((pairLemma3 {a1 = north} (merid x1 ∙ q) (sym q))) ∣
                                          ≡ transport (λ i → uncurry (CODE a n iscon) (pair⁼ q (pairLemma3 (merid x1) q ) i)) ∣ x1 , refl ∣
   wowie' y = J (λ y q → transport (λ i → uncurry (CODE a n iscon) (pair⁼ q (transpRCancel (λ i → north ≡ q i) ((merid x1) ∙ q)) i)) ∣ x1 , rUnit (merid x1)  ∙ sym (cong (λ x → merid x1 ∙ x) (lCancel (sym q))) ∙ assocJ (merid x1) q (sym q) ∙ sym ((pairLemma3 {a1 = north} (merid x1 ∙ q) (sym q))) ∣ ≡ transport (λ i → uncurry (CODE a n iscon) (pair⁼ q (pairLemma3 (merid x1) q ) i)) ∣ x1 , refl ∣)
               (transport
      (λ i →
         ...
         (pair⁼ refl
          (transpRCancel (λ i₁ → north ≡ refl i₁) (merid x1 ∙ refl)) i)) ∣ x1 , originalPath ∣
               ...
               transport (λ i → uncurry (CODE a n iscon) (south , (sym (transportRefl (merid x1)) ∙ (pairLemma3 (merid x1) refl)) i)) ∣ x1 , (λ _ → merid x1) ∣
               ≡⟨ (λ j → transport (λ i → uncurry (CODE a n iscon) (pair⁼Refl (pairLemma3 (merid x1) (λ _ → south)) (~ j) i )) ∣ x1 , (λ _ → merid x1) ∣) ⟩
               transport (λ i → uncurry (CODE a n iscon) (pair⁼ refl (pairLemma3 (merid x1) refl) i)) ∣ x1 , (λ _ → merid x1) ∣ ∎)
...
{- We will use transportLemma to move the transport to the other side. Applying Lemma8610Refl, we will get a transport into a path like the one below. funExt-part' - funExt-part''' transform this expression into something more useful -}
                ...
                (λ {A} {B} mx1 r →  (λ k →  funExt⁻ (Lemma296-funRefl {X = X} {A = λ y → x ≡ y} A B k 
                                    ...
                                    (λ k → funExt⁻ (transportRefl A) (transport (λ i → x ≡ x) (mx1 ∙ (λ _ → x))) ∙
                                      ...
                                      (sym (assoc (transportRefl
                                                  ...
                                                  ((λ i → A (pairLemma3 (transport (λ i₁ → x ≡ x) (mx1 ∙ (λ i₁ → x))) (λ i₁ → x) i)))
...
funExt-part''' : ∀ {ℓ ℓ'} {X : Type ℓ} {x y : X} {A : x ≡ x → Type ℓ'} {B : x ≡ y → Type ℓ'} (ma mx1 : x ≡ y)  →
                ...
                funExt⁻ (Lemma296-fun {X = X} {A = λ y → x ≡ y} ma A B (equivTest {X = X} ma
                                                                                          {A = A}
                                                                                          {B = B}
                                                                                         r))
                         (transport (λ i → x ≡ ma i) (mx1 ∙ sym ma))
...
{- We will need RlFun2 (merid x1) to send ∣ x1 , (λ _ → σ a x1) ∣ to ∣ x1 , refl ∣ -}
funPart : (n : ℕ) (a x1 : A) (iscon : is- (ℕ→ℕ₋₂ n) -ConnectedType A) → RlFun2 a a n iscon (merid x1) ∣ x1 , (λ _ → σ a x1) ∣ ≡ ∣ x1 , refl ∣
...
outerTranspId3' : (n : ℕ) (a x1 : A) (iscon : is- (ℕ→ℕ₋₂ n) -ConnectedType A) →
                ...
                ≡ ∣ x1 , refl ∣
outerTranspId3' {ℓ}{A = A} n a x1 iscon = transportLemma {B = uncurry (CODE a n iscon)}
                                         ...
                                         ∙ (λ k → transport (λ i → uncurry (CODE a n iscon)
                                           ...
                                           (λ k → transport (λ i → uncurry (CODE a n iscon)
                                                  ...
                                                  ∣ x1 , (λ _ → σ a x1) ∣) ∙
                                           -- cong (λ x → x ∣ x1 , (λ _ → σ a x1) ∣) (Lemma8610Refl (CODE' {a = a} n iscon) (merid a) (σ a x1)) ∙
                                                              ...
                                                              (transpRCancel
                                                                 ...
                                                                 funExt⁻ (Lemma296-fun {X = Susp A} {A = λ y → north ≡ y} {B = λ _ → Type ℓ}
                                                                                      ...
                                                                                      (equivTest {X = Susp A} (merid a)
                                                                                       ...
                                                                                       (transport (λ i → north ≡ merid a i) (σ a x1)))
                                                             ∣ x1 , ((λ _ → σ a x1)) ∣) ∙
                                           (λ k → transport (funExt-part''' {A = λ p → ∥ fiber (λ y → σ a y) p ∥ ℕ→ℕ₋₂ (n + n)}
                                                                          {B =  λ q → ∥ fiber merid q ∥ ℕ→ℕ₋₂ (n + n)}
                                                                          (merid a) (merid x1)
                                                                          (λ q → ua' (RlFun2 a a n iscon q , RlFunEquiv2 a a n iscon q)) k)
                                                            ∣ x1 , (((λ _ → σ a x1))) ∣) ∙
  ...
  guyid = pairLemma3 {a1 = north} (σ a x1) (merid a) ∙ sym (assoc (merid x1) (sym (merid a)) (merid a)) ∙
  ...
  finalStep :  transport (cong (λ q → ∥ fiber merid q ∥ ℕ→ℕ₋₂ (n + n)) (sym (pairLemma3 ((merid x1) ∙ sym (merid a)) (merid a) ∙                                                   sym (assocJ (merid x1) (sym (merid a)) (merid a)) ∙ (λ i → (merid x1) ∙ lCancel (merid a) i) ∙ sym (rUnit (merid x1))))) ∣ x1 , refl ∣
                               ≡ ∣ x1 , rUnit (merid x1) ∙ sym (cong (λ x → merid x1 ∙ x) (lCancel (merid a))) ∙
                                                                                   assocJ (merid x1) (sym (merid a)) (merid a) ∙
                                                                                   sym (pairLemma3 {a1 = north} (merid x1 ∙ (sym (merid a))) (merid a)) ∣
...
Thm864 {ℓ} {A} n a iscon {y = y} = J {ℓ} {Susp A} {north} (λ y p → (isContr (CODE a n iscon y p))) (c n a iscon north refl , (λ y → sym (mainId refl y)))
  where
  mainId : (p : north ≡ north) (d : CODE a n iscon north p) →
           d ≡ c n a iscon north p
  mainId p = ind (λ x → isOfHLevelSuc (suc (n + n)) (isOfHLevel∥∥ {A = fiber (λ y → σ a y) p} (ℕ→ℕ₋₂ (n + n)) x (c n a iscon north p)))
                 base
    where
...
Freudenthal n A iscon = Lemma7-5-14 _  (σ (pt A)) (Thm864 n (pt A) iscon)
