...
module Cubical.ZCohomology.FreudenthalCleanup.8-6-2 where
...
open import Cubical.ZCohomology.FreudenthalCleanup.8-6-1
...
WedgeConn {ℓ = ℓ} {ℓ' = ℓ'} A B n zero conA conB P f g p =
  ...
  where
  helper : (a : typ A) → isOfHLevel (suc n) (fiber (inducedFun {A = Unit} {B = typ B} {k = (ℕ→ℕ₋₂ (n + zero))} (pt-map B) (P a)) λ tt → (f a))
  ...
  conOfQ a = transport (λ i → isOfHLevel (2+ pred₋₂ (ℕ→ℕ₋₂ n)) (prelims.QisFib2 A B n zero conA conB P f g p a (~ i)))
                       (transport (λ i → isOfHLevel (natHelper n (~ i)) (prelims.QisFib2 A B n zero conA conB P f g p a i1))
                                  (helper a))
    where
...
WedgeConn {ℓ = ℓ} {ℓ' = ℓ'} A B n (suc m) conA conB P f g p =
          (λ a b → ((typesQ (pt A) .fst) a) .fst b) ,
          ((((λ a → sym ((typesQ (pt A)) .fst a .snd)))) ,
           (λ b → cong (λ x → (x .fst) b) (((typesQ) (pt A)) .snd))) ,
          (sym (transpHelper _ _ _ (cong (λ x → x .snd) (((typesQ) (pt A)) .snd))))
  where
  helper : (a : typ A) → isOfHLevel (suc n) (fiber (inducedFun {A = Unit} {B = typ B} {k = (ℕ→ℕ₋₂ (n + (suc m)))} (pt-map B) (P a)) λ tt → (f a))
  ...
  conOfQ a = transport (λ i → isOfHLevel (natHelper n i) (prelims.Q A B n (suc m) conA conB P f g p a) ) (transport (λ i → isOfHLevel (suc n) (prelims.QisFib2 A B n (suc m) conA conB P f g p a (~ i))) (helper a))
    where
