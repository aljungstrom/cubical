{-# OPTIONS --safe --no-exact-split --experimental-lossy-unification #-}
...
natTranspLem {A = A} n m a B f =
  J (λ m p → f m (subst A p a) ≡ subst B p (f n a))
    (cong (f n) (substRefl a) ∙ sym (substRefl (f n a)))
...
flipSquare≡cong-sym : ∀ {ℓ} {A : Type ℓ} {x : A} → (P : Square (refl {x = x}) refl refl refl) → flipSquare P ≡ λ i j → P i (~ j)
...
-ₖ-helper : {k : ℕ} (n m : ℕ)
  → is-even n ⊎ is-odd n → is-even m ⊎ is-odd m
...
private
  lem₁ : (n : ℕ) (a : _)
    → (cong (cong (subst coHomK (+'-comm (suc zero) (suc (suc n)))))
          (Kn→Ω²Kn+2 ∣ a ∣ₕ))
     ≡ ΩKn+1→Ω²Kn+2
       (sym (testP n) ∙∙ cong (subst coHomK (+'-comm (suc zero) (suc n))) (Kn→ΩKn+1 (suc n) ∣ a ∣ₕ) ∙∙ testP n)
  lem₁ zero a =
  ...
  lem₃ : (n m : ℕ) (q : _) (p : is-even (suc (suc n)) ⊎ is-odd (suc (suc n))) (x : _)
  ...
  lem₃ n m q p x =
    ...
    h n m (inl p) (inl q) x =
         (λ k → cong (-ₖ-gen (suc n) (suc (suc m))
                  (evenOrOdd-Prop (suc n) (evenOrOdd (suc n)) (inr p) k) (inl q))
                    (help n m x k))
    ...
    h n m (inl p) (inr q) x =
       (λ k → cong (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd-Prop (suc n) (evenOrOdd (suc n)) (inr p) k) (inr q))
           (help n m x k))
     ...
     ∙∙ sym (Kn→ΩKn+1-ₖ' (suc n) (suc (suc m)) p q
      ...
      ∙ λ k → Kn→ΩKn+1 (suc (n + suc m))
        (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd-Prop (suc n) (inr p) (evenOrOdd (suc n)) k) (inr q)
         (subst coHomK (cong suc (+-comm (suc m) n)) x))
    h n m (inr p) (inl q) x =
         (λ k → cong (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd-Prop (suc n) (evenOrOdd (suc n)) (inl p) k) (inl q))
                   (help n m x k))
    ...
    h n m (inr p) (inr q) x =
         (λ k → cong (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd-Prop (suc n) (evenOrOdd (suc n)) (inl p) k) (inr q))
                    (help n m x k))
      ...
      ∙∙ cong (Kn→ΩKn+1 (suc (n + suc m)))
           (sym (-ₖ-gen-inl-left (suc n) (suc (suc m)) p (inr q)
             (subst coHomK (λ i → suc (+-comm (suc m) n i)) x)))
       ∙ λ k → Kn→ΩKn+1 (suc (n + suc m))
        (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd-Prop (suc n) (inl p) (evenOrOdd (suc n)) k) (inr q)
         (subst coHomK (cong suc (+-comm (suc m) n)) x))

  lem₄ : (n m : ℕ) (q : _) (p : is-even (suc (suc n)) ⊎ is-odd (suc (suc n))) (a : _) (b : _)
  ...
  lem₅ : (n m : ℕ) (p : _) (q : _) (a : _) (b : _)
    → cong (cong (-ₖ-gen (suc (suc n)) (suc (suc m)) p q
         ...
         (ΩKn+1→Ω²Kn+2 (sym (cong (-ₖ-gen (suc m) (suc (suc n)) (evenOrOdd (suc m)) p) (testP2 n m))
                     ∙∙ (λ i → -ₖ-gen (suc m) (suc (suc n)) (evenOrOdd (suc m)) p
                                   (subst coHomK (+'-comm (suc (suc n)) (suc m))
                                     (_⌣ₖ_ {n = suc (suc n)} {m = suc m} ∣ merid a i ∣ₕ ∣ b ∣ₕ)))
                     ∙∙ cong (-ₖ-gen (suc m) (suc (suc n)) (evenOrOdd (suc m)) p) (testP2 n m)))
  ...
  lem₅ n m p q a b =
    ...
    annoying x =
      ...
      ∙∙ cong (transport (λ i → refl {x = 0ₖ ((+'-comm (suc (suc m)) (suc (suc n))) i)}
              ...
              (natTranspLem {A = coHomK} _ _ x _ (λ _ → Kn→Ω²Kn+2) (cong suc (+-comm (suc n) m)))
      ...
      ∙∙ (λ k → subst (λ n → refl {x = 0ₖ n} ≡ refl {x = 0ₖ n})
                 ...
                 (Kn→Ω²Kn+2 x))
      ∙∙ sym (natTranspLem {A = coHomK} _ _ x _ (λ _ → Kn→Ω²Kn+2) (cong suc (sym (+-suc n m))))
                  ...
                  (λ i j → subst coHomK (+'-comm (suc (suc m)) (suc (suc n)))
                     (Kn→Ω²Kn+2
                       (-ₖ-gen (suc m) (suc (suc n))
                         (evenOrOdd-Prop (suc m) (evenOrOdd (suc m)) (inl y) k) (inr x)
                          (subst coHomK (cong suc (+-comm (suc n) m)) z)) i j)) k)
    ...
    mainHelp z (inl x) (inl y) =
      ...
      ∙∙ λ k → Kn→ΩKn+1 (suc (n + suc m))
        (-ₖ-gen-inl-right (suc (suc n)) (suc (suc m)) (inl x) y
         (-ₖ-gen-inl-right (suc m) (suc (suc n)) (evenOrOdd (suc m)) x
          (-ₖ-gen (suc n) (suc m)
            (evenOrOdd-Prop (suc n) (inr x) (evenOrOdd (suc n)) k)
            (evenOrOdd-Prop (suc m) (inr y) (evenOrOdd (suc m)) k) z) (~ k)) (~ k))
    mainHelp z (inl x) (inr y) =
         (λ k → sym (Kn→ΩKn+1 (suc (n + suc m))
                      (-ₖ-gen (suc n) (suc (suc m))
                        (evenOrOdd-Prop (suc n) (evenOrOdd (suc n)) (inr x) k) (inr y) z)))
       ...
       ∙ λ k → Kn→ΩKn+1 (suc (n + suc m))
        (-ₖ-gen-inl-left (suc (suc n)) (suc (suc m)) x (inr y)
         (-ₖ-gen-inl-right (suc m) (suc (suc n)) (evenOrOdd (suc m)) x
          (-ₖ-gen (suc n) (suc m)
            (evenOrOdd-Prop (suc n) (inr x) (evenOrOdd (suc n)) k)
            (evenOrOdd-Prop (suc m) (inl y) (evenOrOdd (suc m)) k) z) (~ k)) (~ k))
    mainHelp z (inr x) (inl y) =
      ...
      ∙∙ λ k → Kn→ΩKn+1 (suc (n + suc m))
                 (-ₖ-gen-inl-right (suc (suc n)) (suc (suc m)) (inr x) y
                  (-ₖ-gen (suc m) (suc (suc n))
                    (evenOrOdd-Prop (suc m) (inr y) (evenOrOdd (suc m)) k) (inr x)
                   (-ₖ-gen (suc n) (suc m)
                     (evenOrOdd-Prop (suc n) (inl x) (evenOrOdd (suc n)) k)
                     (evenOrOdd-Prop (suc m) (inr y) (evenOrOdd (suc m)) k) z)) (~ k))
    mainHelp z (inr x) (inr y) =
         ((λ k → sym (Kn→ΩKn+1 (suc (n + suc m))
                      (-ₖ-gen (suc n) (suc (suc m))
                        (evenOrOdd-Prop (suc n) (evenOrOdd (suc n)) (inl x) k) (inr y) z))))
      ...
      ∙∙ (λ k → sym (Kn→ΩKn+1 (suc (n + suc m))
       ...
       ∙ λ k → Kn→ΩKn+1-ₖ' (suc (suc n)) (suc (suc m)) x y
                  (-ₖ-gen (suc m) (suc (suc n)) (evenOrOdd-Prop (suc m) (inl y) (evenOrOdd (suc m)) k) (inr x)
                   (-ₖ-gen (suc n) (suc m)
                     (evenOrOdd-Prop (suc n) (inl x) (evenOrOdd (suc n)) k)
                     (evenOrOdd-Prop (suc m) (inl y) (evenOrOdd (suc m)) k)  z)) (~ k)
  ...
  lem₇ (suc n) a (inl x) (inr tt) =
      ((λ k → rUnit (cong (Kn→ΩKn+1 _)
        (λ i → -ₖ-gen (suc (suc n)) (suc zero)
                 (evenOrOdd-Prop n (evenOrOdd (suc (suc n))) (inr x) k) (inr tt)
  ...
  lem₇ (suc n) a (inr x) (inr tt) =
       (λ k → rUnit (λ i j → Kn→ΩKn+1 _
                  (-ₖ-gen (suc (suc n)) (suc zero)
                          (evenOrOdd-Prop (suc (suc n)) (evenOrOdd (suc (suc n))) (inl x) k) (inr tt)
                     (subst coHomK (+'-comm (suc zero) (suc (suc n)))
...
gradedComm-elimCase-left : (n : ℕ) (p : _) (q : _) (a : _) (b : S¹) →
        ...
        (hcomp (λ j → λ {(i = i0) → ∣ north ∣
                        ...
                        ; (k = i0) → (sym (Kn→ΩKn+10ₖ _)
                                    ∙ (λ j → Kn→ΩKn+1 _
                                        ...
                                        ∙ cong (-ₖ-gen (suc n) 1 (evenOrOdd (suc n)) (inr tt)) (testP n)) j))) j i
                        ; (k = i1) → ∣ north ∣})
               ∣ north ∣)
  ...
  hcomp (λ r →
   ...
   (-ₖ-gen (suc (suc n)) 1 p q
    ...
    P i = Kn→ΩKn+1 (suc (suc (n + 0)))
            ((sym (gradedComm-elimCase-left n (evenOrOdd (suc n)) (inr tt) a base
              ∙ cong (-ₖ-gen (suc n) 1 (evenOrOdd (suc n)) (inr tt)) (testP n)) i))
...
gradedComm-elimCase (suc (suc k)) (suc n) (suc m) term p q north (merid a i) r =
  -ₖ-gen (suc (suc n)) (suc (suc m)) p q (
    (subst coHomK (+'-comm (suc (suc m)) (suc (suc n))))
      ((sym (Kn→ΩKn+10ₖ _)
      ∙ cong (Kn→ΩKn+1 _)
          (cong (-ₖ-gen (suc m) (suc (suc n)) (evenOrOdd (suc m)) p) (sym (testP2 n m))
...
gradedComm-elimCase (suc (suc k)) (suc n) (suc m) term p q (merid a i) (merid b j) r =
     ...
     λ { (i = i0) → -ₖ-gen (suc (suc n)) (suc (suc m)) p q
       ...
       ; (r = i1) → -ₖ-gen (suc (suc n)) (suc (suc m)) p q
                      (subst coHomK (+'-comm (suc (suc m)) (suc (suc n)))
                        (doubleCompPath-filler
                         (sym (Kn→ΩKn+10ₖ _))
                          (λ i j → Kn→ΩKn+1 _ ((sym (cong (-ₖ-gen (suc m) (suc (suc n)) (evenOrOdd (suc m)) p) (testP2 n m))
                                             ∙∙ (λ i → -ₖ-gen (suc m) (suc (suc n)) (evenOrOdd (suc m)) p
                                                           (subst coHomK (+'-comm (suc (suc n)) (suc m))
                                                             (_⌣ₖ_ {n = suc (suc n)} {m = suc m} ∣ merid a i ∣ₕ ∣ b ∣ₕ)))
  ...
  help₁ l i j =
    Kn→ΩKn+1 _
      (hcomp (λ r
        → λ { (i = i0) → compPath-filler' (cong ((-ₖ-gen (suc m) (suc (suc n)) (evenOrOdd (suc m)) p)) (sym (testP2 n m)))
                            (sym (gradedComm-elimCase (suc k) m (suc n) (+-suc m n ∙ +-comm (suc m) n ∙ cong predℕ term)
  ...
  help₂ l i j =
    hcomp (λ r →
      λ { (i = i0) → ∣ north ∣
         ...
         ; (j = i0) →
           Kn→ΩKn+1 (suc (suc (n + suc m)))
                (compPath-filler (gradedComm-elimCase (suc k) n (suc m)
                  ...
                  (cong (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd (suc n)) q)  (testP2 m n)) r (~ l)) i
         ; (j = i1) →
              Kn→ΩKn+1 (suc (suc (n + suc m)))
                (compPath-filler (gradedComm-elimCase (suc k) n (suc m)
         ...
         ; (l = i0) →
           Kn→ΩKn+1 _
             (doubleCompPath-filler (sym (cong (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd (suc n)) q) (testP2 m n)))
                                    (λ j → -ₖ-gen (suc n) (suc (suc m)) (evenOrOdd (suc n)) q
                                             (subst coHomK (+'-comm (suc (suc m)) (suc n))
                                               (_⌣ₖ_ {n = suc (suc m)} {m = (suc n)} ∣ merid b j ∣ₕ ∣ a ∣)))
                                    (cong (-ₖ-gen (suc n) (suc (suc m)) (evenOrOdd (suc n)) q) (testP2 m n)) r j) i
...
gradedComm-main : (n m : ℕ) (p : _) (q : _) (a : _)
    → ⌣ₖ∙ (suc n) (suc m) a
...
⌣ₖ-gradedComm : (n m : ℕ) (a : coHomK n) (b : coHomK m)
  → a ⌣ₖ b ≡ (-ₖ^ n · m) (subst coHomK (+'-comm m n) (b ⌣ₖ a))
⌣ₖ-gradedComm zero zero a b = sym (transportRefl _) ∙ cong (transport refl) (comm-·₀ a b) 
⌣ₖ-gradedComm zero (suc m) a b =
      sym (transportRefl _)
...
⌣ₖ-gradedComm (suc n) (suc m) a b =
  funExt⁻ (cong fst (gradedComm-main n m (evenOrOdd (suc n)) (evenOrOdd (suc m)) a)) b
