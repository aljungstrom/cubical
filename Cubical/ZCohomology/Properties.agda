...
rUnitlUnitGen2 : ∀ {ℓ ℓ'} {A : Type ℓ} (n : ℕ) {B : Type ℓ'} {b : B} (e : Iso (hLevelTrunc (suc n) (Susp A)) (hLevelTrunc (suc n) (Path (Susp (Susp A)) north north)))
              ...
              → Path _
                    ...
                    ∙∙ q)
                     ((λ i → inv' e ∣ rCancel (merid north) i ∙ ϕ north north ∣)
...
{-
((λ i →
          ΩKn+1→Kn (suc (suc n)) ∣ ϕ north north ∙ rCancel (merid north) i ∣)
       ∙∙ (λ i → ΩKn+1→Kn (suc (suc n)) ∣ rUnit (ϕ north north) (~ i) ∣)
       ∙∙
       leftInv (Cubical.ZCohomology.KcompPrelims.anIso n)
       (pt ((∥ Susp (S₊ (suc n)) ∥ suc (suc (suc (suc n)))) , ∣ north ∣)))
      ≡
      ((λ i →
          ΩKn+1→Kn (suc (suc n)) ∣ rCancel (merid north) i ∙ ϕ north north ∣)
       ∙∙ (λ i → ΩKn+1→Kn (suc (suc n)) ∣ lUnit (ϕ north north) (~ i) ∣)
       ∙∙
       leftInv (Cubical.ZCohomology.KcompPrelims.anIso n)
       (pt ((∥ Susp (S₊ (suc n)) ∥ suc (suc (suc (suc n)))) , ∣ north ∣)))
-}
...
∙K : (n : ℕ) → loopSpaceK n → loopSpaceK n → loopSpaceK n
∙K zero = _∙_
∙K (suc n) = map2 _∙_

symK : (n : ℕ) → loopSpaceK n → loopSpaceK n
symK zero = sym
symK (suc n) = trMap sym

lUnit∙K : (n : ℕ) (p : loopSpaceK n) → p ≡ ∙K n (reflK n) p
lUnit∙K zero = lUnit
lUnit∙K (suc n) = trElim (λ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _) λ p i → ∣ lUnit p i ∣

rUnit∙K : (n : ℕ) (p : loopSpaceK n) → p ≡ ∙K n p (reflK n)
rUnit∙K zero = rUnit
rUnit∙K (suc n) = trElim (λ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _) λ p i → ∣ rUnit p i ∣

rCancel∙K : (n : ℕ) (p : loopSpaceK n) → ∙K n p (symK n p) ≡ reflK n
rCancel∙K zero = rCancel
rCancel∙K (suc n) = trElim (λ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _) λ p i → ∣ rCancel p i ∣

lCancel∙K : (n : ℕ) (p : loopSpaceK n) → ∙K n (symK n p) p ≡ reflK n
lCancel∙K zero = lCancel
lCancel∙K (suc n) = trElim (λ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _) λ p i → ∣ lCancel p i ∣

assoc∙K : (n : ℕ) (p q r : loopSpaceK n) → ∙K n p (∙K n q r) ≡ ∙K n (∙K n p q) r
assoc∙K zero = assoc
assoc∙K (suc n) = trElim3 (λ _ _ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _)
                           λ p q r i → ∣ assoc p q r i ∣

symDistr∙K : (n : ℕ) (p q : loopSpaceK n) → symK n (∙K n p q) ≡ ∙K n (symK n q) (symK n p) 
symDistr∙K zero = symDistr
symDistr∙K (suc n) = elim2 (λ _ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _)
                           λ p q i → ∣ symDistr p q i ∣

symKsymK : (n : ℕ) (p : loopSpaceK n) → symK n (symK n p) ≡ p
symKsymK zero p = refl
symKsymK (suc n) = trElim (λ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _) λ _ → refl
...
_+ₖ_ {n = n} x y  = ΩKn+1→Kn n (∙K n (Kn→ΩKn+1 n x) (Kn→ΩKn+1 n y))

-ₖ_ : {n : ℕ} →  coHomK n → coHomK n
-ₖ_ {n = n} x = ΩKn+1→Kn n (symK n (Kn→ΩKn+1 n x))

-- subtraction as a binary operator
_-ₖ_ : {n : ℕ} → coHomK n → coHomK n → coHomK n
_-ₖ_  {n = n} x y  = ΩKn+1→Kn n (∙K n (Kn→ΩKn+1 n x) (symK n (Kn→ΩKn+1 n y)))
...
Kn→ΩKn+10ₖ : (n : ℕ) → Kn→ΩKn+1 n (0ₖ n) ≡ reflK n
Kn→ΩKn+10ₖ zero = refl
Kn→ΩKn+10ₖ (suc zero) i = ∣ rCancel (merid base) i ∣
Kn→ΩKn+10ₖ (suc (suc n)) i = ∣ rCancel (merid north) i ∣

ΩKn+1→Kn-refl : (n : ℕ) → ΩKn+1→Kn n (reflK n) ≡ 0ₖ n
...
-0ₖ {n = zero} = refl
-0ₖ {n = suc zero} = refl
-0ₖ {n = suc (suc zero)} = refl
-0ₖ {n = suc (suc (suc zero))} = refl
-0ₖ {n = suc (suc (suc (suc zero)))} = refl
-0ₖ {n = suc (suc (suc (suc (suc n))))} = refl

+ₖ→∙ : (n : ℕ) (a b : coHomK n) → Kn→ΩKn+1 n (a +[ n ]ₖ b) ≡ ∙K n (Kn→ΩKn+1 n a) (Kn→ΩKn+1 n b)
+ₖ→∙ n a b = Iso.rightInv (Iso-Kn-ΩKn+1 n) (∙K n (Kn→ΩKn+1 n a) (Kn→ΩKn+1 n b))
...
lUnitₖ (suc (suc n)) x =
     (λ i → ΩKn+1→Kn (2 + n) (∙K (2 + n) (Kn→ΩKn+10ₖ (2 + n) i) (Kn→ΩKn+1 (2 + n) x)))
  ∙∙ cong (ΩKn+1→Kn (2 + n)) (sym (lUnit∙K (2 + n) (Kn→ΩKn+1 (2 + n) x)))
...
rUnitₖ (suc (suc n)) x =
    (λ i → ΩKn+1→Kn (2 + n) (∙K (2 + n) (Kn→ΩKn+1 (2 + n) x) (Kn→ΩKn+10ₖ (2 + n) i)))
  ∙∙ (cong (ΩKn+1→Kn (2 + n)) (sym (rUnit∙K (2 + n) (Kn→ΩKn+1 (2 + n) x))))
  ∙∙ Iso.leftInv (Iso-Kn-ΩKn+1 (2 + n)) x

rCancelₖ  : (n : ℕ) (x : coHomK n) → x +[ n ]ₖ (-[ n ]ₖ x) ≡ (0ₖ n)
rCancelₖ zero x = (λ i → ΩKn+1→Kn 0 (∙K 0 (Kn→ΩKn+1 0 x) (Iso.rightInv (Iso-Kn-ΩKn+1 0) (symK 0 (Kn→ΩKn+1 0 x)) i))) ∙
                        cong (ΩKn+1→Kn 0) (rCancel∙K 0 (Kn→ΩKn+1 0 x))
rCancelₖ (suc n) x = (λ i → ΩKn+1→Kn (1 + n) (∙K (1 + n) (Kn→ΩKn+1 (1 + n) x) (Iso.rightInv (Iso-Kn-ΩKn+1 (1 + n)) (symK (1 + n) (Kn→ΩKn+1 (1 + n) x)) i))) ∙
                               cong (ΩKn+1→Kn (1 + n)) (rCancel∙K (1 + n) (Kn→ΩKn+1 (1 + n) x)) ∙
                               (λ i → ΩKn+1→Kn (1 + n) (Kn→ΩKn+10ₖ (1 + n) (~ i))) ∙
                               Iso.leftInv (Iso-Kn-ΩKn+1 (1 + n)) (0ₖ (1 + n))

lCancelₖ : (n : ℕ) (x : coHomK n) → (-[ n ]ₖ x) +[ n ]ₖ x  ≡ (0ₖ n)
lCancelₖ 0 x = (λ i → ΩKn+1→Kn 0 (∙K 0 (Iso.rightInv (Iso-Kn-ΩKn+1 0) (symK 0 (Kn→ΩKn+1 0 x)) i) (Kn→ΩKn+1 0 x))) ∙
                        cong (ΩKn+1→Kn 0) (lCancel∙K 0 (Kn→ΩKn+1 0 x))
lCancelₖ (suc n) x = (λ i → ΩKn+1→Kn (1 + n) (∙K (1 + n) (Iso.rightInv (Iso-Kn-ΩKn+1 (1 + n)) (symK (1 + n) (Kn→ΩKn+1 (1 + n) x)) i) (Kn→ΩKn+1 (1 + n) x))) ∙
                               cong (ΩKn+1→Kn (1 + n)) (lCancel∙K (1 + n) (Kn→ΩKn+1 (1 + n) x)) ∙
                               (λ i → (ΩKn+1→Kn (1 + n)) (Kn→ΩKn+10ₖ (1 + n) (~ i))) ∙
                               Iso.leftInv (Iso-Kn-ΩKn+1 (1 + n)) (0ₖ (1 + n))

assocₖ : (n : ℕ) (x y z : coHomK n) → ((x +[ n ]ₖ y) +[ n ]ₖ z) ≡ (x +[ n ]ₖ (y +[ n ]ₖ z))
assocₖ n x y z = ((λ i → ΩKn+1→Kn n (∙K n (Kn→ΩKn+1 n (ΩKn+1→Kn n (∙K n (Kn→ΩKn+1 n x) (Kn→ΩKn+1 n y)))) (Kn→ΩKn+1 n z))) ∙∙
                          (λ i → ΩKn+1→Kn n (∙K n (Iso.rightInv (Iso-Kn-ΩKn+1 n) (∙K n (Kn→ΩKn+1 n x) (Kn→ΩKn+1 n y)) i) (Kn→ΩKn+1 n z))) ∙∙
                          (λ i → ΩKn+1→Kn n (assoc∙K n (Kn→ΩKn+1 n x) (Kn→ΩKn+1 n y) (Kn→ΩKn+1 n z) (~ i)))) ∙
                          (λ i → ΩKn+1→Kn n (∙K n (Kn→ΩKn+1 n x) (Iso.rightInv (Iso-Kn-ΩKn+1 n) ((∙K n (Kn→ΩKn+1 n y) (Kn→ΩKn+1 n z))) (~ i))))

cancelₖ : (n : ℕ) (x : coHomK n) → x -[ n ]ₖ x ≡ (0ₖ n)
cancelₖ zero x = cong (ΩKn+1→Kn 0) (rCancel∙K 0 (Kn→ΩKn+1 zero x))
cancelₖ (suc zero) x = cong (ΩKn+1→Kn 1) (rCancel∙K 1 (Kn→ΩKn+1 1 x))
cancelₖ (suc (suc zero)) x = cong (ΩKn+1→Kn 2) (rCancel∙K 2 (Kn→ΩKn+1 2 x))
cancelₖ (suc (suc (suc zero))) x = cong (ΩKn+1→Kn 3) (rCancel∙K 3 (Kn→ΩKn+1 3 x))
cancelₖ (suc (suc (suc (suc zero)))) x = cong (ΩKn+1→Kn 4) (rCancel∙K 4 (Kn→ΩKn+1 4 x))
cancelₖ (suc (suc (suc (suc (suc n))))) x = cong (ΩKn+1→Kn (5 + n)) (rCancel∙K (5 + n) (Kn→ΩKn+1 (5 + n) x))

-rUnitₖ : (n : ℕ) (x : coHomK n) → x -[ n ]ₖ 0ₖ n ≡ x
-rUnitₖ zero x = rUnitₖ zero x
-rUnitₖ (suc n) x = cong (λ y → ΩKn+1→Kn (suc n) (∙K (suc n) (Kn→ΩKn+1 (suc n) x) (symK (suc n) y))) (Kn→ΩKn+10ₖ (suc n))
                 ∙∙ cong (ΩKn+1→Kn (suc n)) (sym (rUnit∙K (suc n) (Kn→ΩKn+1 (suc n) x)))
...
private
  ...
  helper : (n : ℕ) → Iso.inv (Iso-Kn-ΩKn+1 (2 + n)) ((Iso.fun (PathIdTruncIso (4 + n))) refl) ≡ ∣ north ∣
  helper zero = refl
  helper (suc n) = refl

isCommΩK : (n : ℕ) → isComm∙ (coHomK-ptd (2 + n))
isCommΩK n =
  subst isComm∙ (λ i → coHomK (2 + n) , helper n i)
                (ptdIso→comm {A = (_ , _)}
                  (compIso (PathIdTruncIso (4 + n)) (invIso (Iso-Kn-ΩKn+1 (2 + n))))
                    (Eckmann-Hilton 0))

isCommLoopSpace : (n : ℕ) → (p q : loopSpaceK n) → ∙K n p q ≡ ∙K n q p
isCommLoopSpace zero = comm-ΩS¹
isCommLoopSpace (suc n) =
  elim2 (λ _ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _)
    λ p q → sym (Iso.rightInv (PathIdTruncIso (3 + n)) (∣ p ∙ q ∣))
          ∙∙ cong (Iso.fun (PathIdTruncIso (3 + n))) (congFunct ∣_∣ p q
                                                  ∙∙ (isCommΩK n (cong ∣_∣ p) (cong ∣_∣ q))
                                                  ∙∙ (sym (congFunct ∣_∣ q p)))
          ∙∙ Iso.rightInv (PathIdTruncIso (3 + n)) (∣ q ∙ p ∣)

commₖ : (n : ℕ) (x y : coHomK n) → (x +[ n ]ₖ y) ≡ (y +[ n ]ₖ x)
commₖ zero a b =
  cong (ΩKn+1→Kn 0) (comm-ΩS¹ (intLoop a) (intLoop b))
commₖ (suc n) x y =
  cong (ΩKn+1→Kn (suc n)) (isCommLoopSpace (suc n) (Kn→ΩKn+1 (suc n) x) (Kn→ΩKn+1 (suc n) y))

-distrₖ : (n : ℕ) (x y : coHomK n) → -[ n ]ₖ (x +[ n ]ₖ y) ≡ (-[ n ]ₖ x) +[ n ]ₖ (-[ n ]ₖ y)
-distrₖ n x y = ((λ i → ΩKn+1→Kn n (symK n (Kn→ΩKn+1 n (ΩKn+1→Kn n (∙K n (Kn→ΩKn+1 n x) (Kn→ΩKn+1 n y)))))) ∙∙
                      (λ i → ΩKn+1→Kn n (symK n (Iso.rightInv (Iso-Kn-ΩKn+1 n) (∙K n (Kn→ΩKn+1 n x) (Kn→ΩKn+1 n y)) i))) ∙∙
                      (λ i → ΩKn+1→Kn n (symDistr∙K n (Kn→ΩKn+1 n x) (Kn→ΩKn+1 n y) i))) ∙∙
                      (λ i → ΩKn+1→Kn n (∙K n (Iso.rightInv (Iso-Kn-ΩKn+1 n) (symK n (Kn→ΩKn+1 n y)) (~ i)) (Iso.rightInv (Iso-Kn-ΩKn+1 n) (symK n (Kn→ΩKn+1 n x)) (~ i)))) ∙∙
                      commₖ n (-[ n ]ₖ y) (-[ n ]ₖ x)

private
  rCancelLem : (n : ℕ) (x : coHomK n) → ΩKn+1→Kn n (∙K n (Kn→ΩKn+1 n x) (reflK n)) ≡ ΩKn+1→Kn n (Kn→ΩKn+1 n x)
  rCancelLem zero x = refl
  rCancelLem (suc n) x = cong (ΩKn+1→Kn (suc n)) (sym (rUnit∙K (suc n) (Kn→ΩKn+1 (suc n) x)))

  lCancelLem : (n : ℕ) (x : coHomK n) → ΩKn+1→Kn n (∙K n (reflK n) (Kn→ΩKn+1 n x)) ≡ ΩKn+1→Kn n (Kn→ΩKn+1 n x)
  lCancelLem zero x = refl
  lCancelLem (suc n) x = cong (ΩKn+1→Kn (suc n)) (sym (lUnit∙K (suc n) (Kn→ΩKn+1 (suc n) x)))


-cancelRₖ : (n : ℕ) (x y : coHomK n) → (y +[ n ]ₖ x) -[ n ]ₖ x ≡ y
-cancelRₖ n x y = (cong (ΩKn+1→Kn n) ((cong (λ y → ∙K n y (symK n (Kn→ΩKn+1 n x))) (+ₖ→∙ n y x))
                                  ∙∙ sym (assoc∙K n _ _ _)
                                  ∙∙ cong (∙K n (Kn→ΩKn+1 n y)) (rCancel∙K n _)))
                   ∙∙ rCancelLem n y
...
-+cancelₖ n x y = (cong (ΩKn+1→Kn n) ((cong (λ x → ∙K n x (Kn→ΩKn+1 n y)) (Iso.rightInv (Iso-Kn-ΩKn+1 n) (∙K n (Kn→ΩKn+1 n x) (symK n (Kn→ΩKn+1 n y)))))
                                  ∙∙ sym (assoc∙K n _ _ _)
                                  ∙∙ cong (∙K n (Kn→ΩKn+1 n x)) (lCancel∙K n _)))
                   ∙∙ rCancelLem n x
...
-ₖ-ₖ n x =
     cong ((ΩKn+1→Kn n) ∘ (symK n)) (Iso.rightInv (Iso-Kn-ΩKn+1 n) (symK n (Kn→ΩKn+1 n x)))
  ∙∙ cong (ΩKn+1→Kn n) (symKsymK n _)
  ∙∙ Iso.leftInv (Iso-Kn-ΩKn+1 n) x
...
open GroupStr
open GroupHom
coHomGrStr : ∀ {ℓ} (n : ℕ) (A : Type ℓ) → GroupStr (coHom n A)
0g (coHomGrStr n A) = 0ₕ n
GroupStr._+_ (coHomGrStr n A) x y = x +[ n ]ₕ y
(- coHomGrStr n A) x = -[ n ]ₕ x
isGroup (coHomGrStr n A) = isGroupCohomStr
  where
  abstract
    isGroupCohomStr : IsGroup (0ₕ n) (λ x y → +ₕ-syntax n x y) (λ x → -ₕ-syntax n x)
    is-set (isSemigroup (IsGroup.isMonoid isGroupCohomStr)) = §
    IsSemigroup.assoc (isSemigroup (IsGroup.isMonoid isGroupCohomStr)) x y z = sym (assocₕ n x y z)
    identity (IsGroup.isMonoid isGroupCohomStr) x = (rUnitₕ n x) , (lUnitₕ n x)
    IsGroup.inverse isGroupCohomStr x = (rCancelₕ n x) , (lCancelₕ n x)

coHomGr : ∀ {ℓ} (n : ℕ) (A : Type ℓ) → Group
fst (coHomGr n A) = coHom n A
snd (coHomGr n A) = coHomGrStr n A
...
+K→∙ : (key : Unit') (n : ℕ) (a b : coHomK n) → Kn→ΩKn+1 n (lockedCohom.+K key n a b) ≡ ∙K n (Kn→ΩKn+1 n a) (Kn→ΩKn+1 n b)
+K→∙ unlock = +ₖ→∙
