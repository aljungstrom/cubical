...
private
  ...
  ∙K zero = rec2 (isOfHLevelTrunc 2) λ p q → ∣ p ∙ q ∣
  ∙K (suc n) = rec2 (isOfHLevelTrunc (3 + n)) λ p q → ∣ p ∙ q ∣
  ...
  lUnit∙K zero = trElim (λ _ → isOfHLevelPath 2 (isOfHLevelTrunc 2) _ _) λ p i → ∣ lUnit p i ∣
  ...
  rUnit∙K zero = trElim (λ _ → isOfHLevelPath 2 (isOfHLevelTrunc 2) _ _) λ p i → ∣ rUnit p i ∣
  ...
  rCancel∙K zero = trElim (λ _ → isOfHLevelPath 2 (isOfHLevelTrunc 2) _ _) λ p i → ∣ rCancel p i ∣
  ...
  lCancel∙K zero = trElim (λ _ → isOfHLevelPath 2 (isOfHLevelTrunc 2) _ _) λ p i → ∣ lCancel p i ∣
  ...
  assoc∙K zero = trElim3 (λ _ _ _ → isOfHLevelPath 2 (isOfHLevelTrunc 2) _ _)
                          λ p q r i → ∣ assoc p q r i ∣
  assoc∙K (suc n) = trElim3 (λ _ _ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _)
                             λ p q r i → ∣ assoc p q r i ∣

  comm∙K : (n : ℕ) (p q : loopSpaceK n) → ∙K n p q ≡ ∙K n q p
  comm∙K zero = elim2 (λ _ _ → isOfHLevelPath 2 (isOfHLevelTrunc 2) _ _) λ p q i → ∣ comm-ΩS¹ p q i ∣
  comm∙K (suc n) = elim2 (λ _ _ → isOfHLevelPath (3 + n) (isOfHLevelTrunc (3 + n)) _ _) λ p q i → ∣ {!!} ∣
    where
    help : Iso {!hLevelTrunc n ?!} {!!}
    help = {!!}
...
{-
isComm∙K : ∀ {ℓ} (A : Pointed ℓ) → Type ℓ
isComm∙K A = (p q : typ (Ω A)) → p ∙ q ≡ q ∙ p
...
commₖ zero a b = cong (ΩKn+1→Kn 0) λ i → ∣ comm-ΩS¹ (intLoop a) (intLoop b) i ∣
commₖ (suc zero) x y = cong (ΩKn+1→Kn 1) {!!} -- cong (ΩKn+1→Kn 1) (ptdIso→comm {A = (_ , _)} (invIso (Iso-Kn-ΩKn+1 2)) (Eckmann-Hilton 0) (Kn→ΩKn+1 1 x) (Kn→ΩKn+1 1 y))
