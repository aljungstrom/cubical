...
_⌣'_ {n = n} {m = m} x y = _⌣_ {n = n} {m = m} (_⌣_ {n = zero} {m = n} (∣ (λ _ → 1) ∣₂) x) ((_⌣_ {n = zero} {m = m} (∣ (λ _ → 1) ∣₂) y))
...
newTest : coHom 2 (S₊ 1 × S₊ 1) → coHom 2 S²⋁S¹⋁S¹
newTest = Cubical.HITs.SetTruncation.map tihi
  where
  tihi : (f : _) → S²⋁S¹⋁S¹ → coHomK 2
  tihi f (inl north) = 0ₖ 2
  tihi f (inl south) = 0ₖ 2
  tihi f (inl (merid base i)) = (sym (lCancelₖ 2 (f (base , base)))
                             ∙∙ (λ i → (-ₖ f (base , base)) +ₖ (f (loop i , base)))
                             ∙∙ lCancelₖ 2 (f (base , base))) i
  tihi f (inl (merid (loop j) i)) =
    hcomp (λ k → λ {(i = i0) → lCancelₖ 2 (f (base , loop j)) k
                   ; (i = i1) → lCancelₖ 2 (f (base , loop j)) k
                   ; (j = i0) → doubleCompPath-filler (sym (lCancelₖ 2 (f (base , base))))
                                                        (λ i → (-ₖ f (base , base)) +ₖ (f (loop i , base)))
                                                        (lCancelₖ 2 (f (base , base))) k i
                   ; (j = i1) → doubleCompPath-filler (sym (lCancelₖ 2 (f (base , base))))
                                                        (λ i → (-ₖ f (base , base)) +ₖ (f (loop i , base)))
                                                        (lCancelₖ 2 (f (base , base))) k i })
          ((-ₖ f (base , loop j)) +ₖ f (loop i , loop j)) -- f (loop i , loop j) +ₖ f (base , loop j) -- f (loop i , loop j) +ₖ f (base , loop j)
  tihi f (inr x) = 0ₖ _
  tihi f (push a i) = 0ₖ _

one : (S₊ 1 × S₊ 1 → coHomK 2) → Σ[ g ∈ (S¹ → coHomK 2) ] ((x : S¹) → g x ≡ g x)
one f = (λ x → f (x , base)) , (λ x → cong (λ y → f (x , y)) loop)

two : ∥ Σ[ g ∈ (S¹ → coHomK 2) ] ((x : S¹) → g x ≡ g x) ∥₂ → ∥ Σ[ g ∈ (S¹ → coHomK 2) ] (∥ ((x : S¹) → g x ≡ g x) ∥₂) ∥₂
two = Cubical.HITs.SetTruncation.map λ p → (fst p) , ∣ snd p ∣₂

three : (g : (S¹ → coHomK 2)) →  ((x : S¹) → g x ≡ g x) → (S¹ → coHomK 1)
three g F x = ΩKn+1→Kn 1 (sym (lCancelₖ 2 (g x)) ∙∙ (λ i →  (-ₖ (g x)) +ₖ (F x i)) ∙∙ lCancelₖ 2 (g x))
 -- Hⁿ-Sⁿ≅ℤ
newTry : coHom 2 (S₊ 1 × S₊ 1) → Int
newTry = sRec isSetInt λ f → Iso.fun (fst (Hⁿ-Sⁿ≅ℤ 0)) ∣ three (one f .fst) (one f .snd) ∣₂

1-torus : coHom 2 (S₊ 1 × S₊ 1)
1-torus = ∣ (λ { (base , base) → 0ₖ 2 ; (loop j , base) →  ∣ merid base j ∣ -ₖ ∣ merid base j ∣ ; (base , loop i) → 0ₖ _ ; (loop j , loop i) → ∣ merid (loop i) j ∣ -ₖ ∣ merid base j ∣}) ∣₂

_⌣''_ : ∀ {ℓ} {A : Type ℓ} {n m : ℕ} → coHom n A → coHom m A → coHom (n +' m) A
_⌣''_ {n = n} {m = m} = Cubical.HITs.SetTruncation.rec2 squash₂ λ f g → ∣ (λ x → (_⌣ₖ_ {n = n} {m = m} (f x -[ n ]ₖ 0ₖ n) (g x -[ m ]ₖ 0ₖ m)) +[ n +' m ]ₖ (0ₖ (n +' m))) ∣₂

-- ola : newTry (((from₁ (1 , 0) ⌣'' from₁ (0 , 1)))) ≡ 1
-- ola = {!!}


-- -ₖ'_ : {n : ℕ} → coHomK n → coHomK n
-- -ₖ'_ {n = zero} = -ₖ_ {n = zero}
-- -ₖ'_ {n = suc zero} = -ₖ_
-- -ₖ'_ {n = suc (suc n)} =
--   trMap λ { north → south
--           ; south → north
