...
module Cubical.ZCohomology.Freudenthal.8-6-1 where
...
Lemma861-fibId {A = A} {B = B} n k f isCon P l (g , p) (h , q)  = helper1 _ _ _  ∙ helper2 _ _ _ ∙ (λ j → Σ ((x : B) → g x ≡ h x) (λ r → helper3 l (g , p) (h , q) r j)) ∙ λ j → Σ ((x : B) → g x ≡ h x) (λ r → helper4 l (g , p) (h , q) r j)
    where
...
Lemma861 {A = A} {B = B} n (suc k) (suc zero) f isCon P pf = λ l → transport (λ i → isOfHLevel (2+ (-2+ (suc k - n))) (Lemma861-fibId2 {A = A} {B = B} (ℕ→ℕ₋₂ n) k f isCon P l (~ i)))
                                                                             (transport (λ i → isOfHLevel (natId (~ i)) (Σ ((b : B) → fst (P b)) λ g → ((a : A) → g (f a) ≡ l a)))
                                                                                        λ a b → transport (λ i → Lemma861-fibId (ℕ→ℕ₋₂ n) k f isCon P l a b (~ i) )
...
Lemma861 {A = A} {B = B} n (suc k) (suc (suc dif)) f isCon P pf = λ l → transport (λ i → isOfHLevel (2+ (-2+ (suc k - n))) (Lemma861-fibId2 {A = A} {B = B} (ℕ→ℕ₋₂ n) k f isCon P l (~ i))) ((transport (λ i → isOfHLevel (natId pf(~ i)) (Σ ((b : B) → fst (P b)) λ g → ((a : A) → g (f a) ≡ l a))) λ a b → transport (λ i → isOfHLevel (suc dif) (Lemma861-fibId (ℕ→ℕ₋₂ n) k f isCon P l a b (~ i)) ) (finalTruncLemma l a b λ x → ((snd a x) ∙ sym (snd b x)))))
  where
...
Lemma861-fibId' : (n : ℕ₋₂) (f : A → B) →
                 (is- n -Connected f) →
                 (P : B → HLevel ℓ (2+ (ℕ→ℕ₋₂ zero)))
                 (l : (a : A) → fst (P (f a))) (a b : Σ ((b : B) → fst (P b)) λ g → (a : A) → g (f a) ≡ l a ) →
                 (a ≡ b) ≡ (fiber ((inducedFun {k = (ℕ→ℕ₋₂ zero) } f (λ x → ((fst a) x ≡ (fst b) x ) , isOfHLevelSuc 2 (P x .snd) (fst a x) (fst b x)))) (λ x → ((snd a) x) ∙ (sym ((snd b) x))))
Lemma861-fibId' {A = A} {B = B} n f isCon P l (g , p) (h , q)  =  helper1 _ _ _  ∙ helper2 _ _ _ ∙ (λ j → Σ ((x : B) → g x ≡ h x) (λ r → helper3 l (g , p) (h , q) r j)) ∙ λ j → Σ ((x : B) → g x ≡ h x) (λ r → helper4 l (g , p) (h , q) r j)
    where
    helper1 : (l : (a : A) → fst (P (f a)))
              (a b : Σ ((b : B) → fst (P b)) λ g → (a : A) → g (f a) ≡ l a ) →
              (a ≡ b) ≡ Σ (fst a ≡ fst b) λ r → PathP (λ i → (x : A) → (r i) (f x) ≡ l x) (snd a) (snd b)
    helper1 l a b = sym (ua (Σ≡ )) 
...
Lemma861-fibId2' : ∀{ℓ} (n k : ℕ₋₂) (f : A → B) →
                 (is-  n -Connected f) →
                 (P : B → HLevel ℓ (2+ (suc₋₂ k))) →
                 (l : (a : A) → fst (P (f a))) →
                 (fiber (inducedFun {k = (suc₋₂ k)} f P ) l) ≡ Σ ((b : B) → fst (P b)) λ g → ((a : A) → g (f a) ≡ l a)
Lemma861-fibId2' n k f isCon P l = isoToPath (iso (λ p → fst p , funExt⁻ (snd p)) (λ p → fst p , funExt (snd p)) (λ b → refl) λ b → refl)


Lemma861neg1neg1 : ∀{ℓ} (f : A → B) →
           (is- neg1 -Connected f) →
           (P : B → HLevel ℓ (2+  neg1)) →
           is- (neg2) -Truncated (inducedFun {k = neg1} f P)
Lemma861neg1neg1 {A = A} {B = B} f iscon P = λ g → equiv-proof (Lemma757i→ii f neg1 iscon P) g


Lemma861neg1 : ∀{ℓ} (k : ℕ) (f : A → B) →
           (is- neg1 -Connected f) →
           (P : B → HLevel ℓ (2+ (ℕ→ℕ₋₂ k))) →
           is- (-2+ (suc k)) -Truncated (inducedFun {k = ℕ→ℕ₋₂ k} f P)
Lemma861neg1 {A = A} {B = B} zero f iscon P = λ l → transport (λ i → isProp (Lemma861-fibId2' neg1 (neg1) f iscon P l (~ i))) λ a b → transport (λ i → Lemma861-fibId' neg1 f iscon P l a b (~ i)) ((finalTruncLemma l a b λ x → ((snd a x) ∙ sym (snd b x))) .fst)
  where

  finalTruncLemma : (l : (a : A) → fst (P (f a))) (a b : Σ ((b : B) → fst (P b)) λ g → (a : A) → g (f a) ≡ l a ) → is- -2+ zero -Truncated ((inducedFun {k = (ℕ→ℕ₋₂ zero)} f (λ x → ((fst a) x ≡ (fst b) x ) , isOfHLevelSuc 2 (snd (P x)) (fst a x) (fst b x)  ) ))
  finalTruncLemma l a b = λ z → Lemma861neg1neg1 f iscon ((λ x → ((fst a) x ≡ (fst b) x ) , (snd (P x)) (fst a x) (fst b x) )) z

Lemma861neg1 {A = A} {B = B} (suc k) f iscon P = λ l → transport (λ i → isOfHLevel (2+ (-1+ (suc k))) (Lemma861-fibId2 {A = A} {B = B} (neg1) k f iscon P l (~ i))) λ a b →  transport (λ i → isOfHLevel (suc k) (Lemma861-fibId neg1 k f iscon P l a b (~ i)) ) (finalTruncLemma l a b λ x → ((snd a x) ∙ sym (snd b x)))
  where

  finalTruncLemma : (l : (a : A) → fst (P (f a))) (a b : Σ ((b : B) → fst (P b)) λ g → (a : A) → g (f a) ≡ l a ) → is- -2+ (suc k) -Truncated ((inducedFun {k = (ℕ→ℕ₋₂ k)} f (λ x → ((fst a) x ≡ (fst b) x ) , (snd (P x)) (fst a x) (fst b x) ) ))
  finalTruncLemma l a b =  λ b₁ → (Lemma861neg1 k f iscon ((λ x → ((fst a) x ≡ (fst b) x ) , (snd (P x)) (fst a x) (fst b x) )) b₁)
