...
rem+quot-mod≡ (suc n) =
  +induction n (λ x → fst (ℕ→rem+quot-mod (suc n) x) + (suc n) · snd (ℕ→rem+quot-mod (suc n) x) ≡ x)
...
-- mereRel→Set : ∀ {ℓ ℓ'} {A : Type ℓ} (R : A → A → Type ℓ')
--             → ((x y : A) → R x y → x ≡ y)
--             → ((x y : A) → isProp (R x y))
--             → isSet A
-- mereRel→Set R p f = {!!}
...
-- rotter : ℕ → S¹ → S¹
-- rotter n base = base
-- rotter n (loop i) = intLoop (pos n) i
-- bz2 : (n  : ℕ)→ Type₀
-- bz2 n = Σ[ x ∈ S¹ ] Σ[ y ∈ S¹ ] rotter n x ≡ y

-- help-f : (n : ℕ) (x : S¹) → bz2 (suc n)
-- help-f x base = base , (base , refl)
-- help-f x (loop i) = loop i , intLoop (pos (suc x)) i , refl
-- bz3 : (n : ℕ) → B-Z/ (suc n)  → bz2 (suc n)
-- bz3 n = B-Z/-elim _ (help-f n) (help n)
--   where
--   help : (n : ℕ) → cong (help-f n) (intLoop (pos n) ∙ loop) ≡ refl
--   help zero = cong (cong (help-f zero)) (sym (lUnit loop)) ∙ (λ j →  {!!}) ∙ {!!}
--   help (suc n) = {!!}

-- back-f : (n : ℕ) → bz2 (suc n) → B-Z/ (suc n)
-- back-f n (base , y , p) = [ base ]
-- back-f n (loop i , y , p) = [ loop i ]

-- retr : (n : ℕ) → (x : _) → back-f n (bz3 n [ x ]) ≡ [ x ]
-- retr n base = refl
-- retr n (loop i) = refl

-- test2 : (n : ℕ) → (x y : (B-Z/ (suc n))) → (p q : x ≡ y) → (c : p ≡ q) → isContr (p ≡ q)
-- fst (test2 n x y p q c) = c
-- snd (test2 n x y p q c) =  {!!}
...
-- --                             λ p → (λ i → [ intLoop (pos (fst p)) i ])
-- --   pathp n =
-- --     toPathP (funExt λ p → (λ k → transport (λ i → Path (B-Z/ (suc n)) [ base ] [ loop i ])
-- --                                              λ i → [ intLoop (pos (fst ((-/ 1f) +/ transportRefl p k))) i ])
