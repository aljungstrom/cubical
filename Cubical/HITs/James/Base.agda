...
JS2→S2 : J (S¹ , base) → hLevelTrunc 2 S¹
JS2→S2 εJ = ∣ base ∣
JS2→S2 (αJ base x₁) = JS2→S2 x₁
JS2→S2 (αJ (loop i) εJ) = ∣ loop i ∣
JS2→S2 (αJ (loop i) (αJ base x₁)) = JS2→S2 (αJ (loop i) x₁)
JS2→S2 (αJ (loop i) (αJ (loop j) x)) = isSet→isSet' (isOfHLevelTrunc 2) (λ j → JS2→S2 (αJ (loop j) x)) (λ j → JS2→S2 (αJ (loop j) x)) (λ j → JS2→S2 (αJ (loop j) x)) (λ j → JS2→S2 (αJ (loop j) x)) i j

-- (λ _ j → JS2→S2 (αJ (loop j) x)) (λ _ j → JS2→S2 (αJ (loop j) x))
   --                                                    (λ j _ → JS2→S2 (αJ (loop j) x)) (λ j _ → JS2→S2 (αJ (loop j) x)) i j
JS2→S2 (αJ (loop i) (δJ x₁ i₁)) = {!!}
JS2→S2 (δJ x i) = JS2→S2 x


-- JS2→S2 : J (S² , base) → hLevelTrunc 4 S²
-- JS2→S2 εJ = ∣ base ∣
-- JS2→S2 (αJ base x₁) = JS2→S2 x₁
-- JS2→S2 (αJ (surf i i₁) εJ) = ∣ surf i i₁ ∣
-- JS2→S2 (αJ (surf i i₁) (αJ base x₁)) = JS2→S2 (αJ (surf i i₁) x₁)
-- JS2→S2 (αJ (surf i i₁) (αJ (surf i₂ i₃) εJ)) = {!!}
-- JS2→S2 (αJ (surf i j) (αJ (surf k l) (αJ base x))) = helper x i j k l
--   where
--   helper : (x : (J (S² , base)))
--     → 4Cube (λ _ k l  → JS2→S2 (αJ (surf k l) x)) (λ _ k l  → JS2→S2 (αJ (surf k l) x))
--              (λ _ k l  → JS2→S2 (αJ (surf k l) x)) (λ _ k l  → JS2→S2 (αJ (surf k l) x))
--              (λ i j _  → JS2→S2 (αJ (surf i j) x)) (λ i j _  → JS2→S2 (αJ (surf i j) x))
--              (λ i j _  → JS2→S2 (αJ (surf i j) x)) (λ i j _  → JS2→S2 (αJ (surf i j) x))
--   helper x = is2Groupoid→is2Groupoid' {!!} _ _ _ _ _ _ _ _ -- is2Groupoid→is2Groupoid' (isOfHLevelTrunc 4) _ _ _ _ _ _ _ _
-- JS2→S2 (αJ (surf i i₁) (αJ (surf i₂ i₃) (αJ (surf i₄ i₅) x₁))) = {!!}
-- JS2→S2 (αJ (surf i i₁) (αJ (surf i₂ i₃) (δJ x i₄))) = {!!}

-- JS2→S2 (αJ (surf i i₁) (δJ x₁ i₂)) = JS2→S2 (αJ (surf i i₁) x₁)
-- JS2→S2 (δJ x i) = JS2→S2 x

-- -- JS2→S2back : S² → hLevelTrunc 4 (J (S² , base))
-- -- JS2→S2back base = ∣ αJ base εJ ∣
-- -- JS2→S2back (surf i i₁) = ∣ (αJ (surf i i₁) εJ) ∣

-- -- theIso : Iso (hLevelTrunc 4 (J (S² , base))) (hLevelTrunc 4 S²)
-- -- Iso.fun theIso = rec (isOfHLevelTrunc 4) JS2→S2
-- -- Iso.inv theIso = rec (isOfHLevelTrunc 4) JS2→S2back
-- -- Iso.rightInv theIso =
...
-- --                     ; (k = i0) → JS2→S2back (JS2→S2 (αJ (surf i j) x))
-- --                     ; (k = i1) → αJ (surf i j) (αJ base x)})
-- --            (hcomp (λ m → λ { (k = i0) → JS2→S2back (JS2→S2 (αJ (surf i j) x))
...
-- -- -- retrH2 : section JS2→S2back JS2→S2
-- -- -- retrH2 εJ = sym (δJ εJ)
...
-- -- -- -- retrH : section JS2→S2back JS2→S2
-- -- -- -- retrH εJ = sym (δJ εJ)
...
-- -- -- --   where
-- -- -- -- retrH (αJ (surf i i₁) (αJ (surf i₂ i₃) x₁)) = {!(cong (cong (λ x → JS2→S2back (JS2→S2 (αJ x x₁)))) surf)!}
