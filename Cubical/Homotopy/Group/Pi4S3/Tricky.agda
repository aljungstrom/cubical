  ...
  trRec (isProp→isOfHLevelSuc 1 isPropIsContr)
    (J (λ p _ → isConnected 2 (fiber f p))
     ...
     , (trElim (λ _ → isOfHLevelPath 2 (isOfHLevelTrunc 2) _ _)
         (uncurry (sphereElim 2 (λ _ → isOfHLevelΠ 3 λ _ → isOfHLevelPath 3 (isOfHLevelSuc 2 (isOfHLevelTrunc 2)) _ _)
           λ p → trRec (isOfHLevelTrunc 2 _ _) (λ r → cong ∣_∣ₕ (ΣPathP (refl , r))) (fun (PathIdTruncIso 1) (isContr→isProp (isConnectedPath 2 (sphereConnected 2) (f north) (f north)) ∣ refl ∣ ∣ p ∣)))))))
...
isConnectedUnitmap : ∀ {ℓ} {A : Type ℓ} (n : ℕ)
  → isConnected n A
  → isConnectedFun n (λ (a : A) → tt)
isConnectedUnitmap n con tt =
  subst (isConnected n) (isoToPath (invIso fibreUnitMap)) con

module BM-inst =
  BlakersMassey□ (λ _ → tt) fold∘W 3
   1
   (isConnectedUnitmap 4 (sphereConnected 3))
   (isConnectedS3→S2 fold∘W)

open BM-inst

Grr : S₊ 3 → Σ (Unit × S₊ 2) PushoutPath×
Grr = toPullback

thePullback∙ : Pointed₀
fst thePullback∙ = Σ (Unit × S₊ 2) PushoutPath×
snd thePullback∙ = (tt , north) , (push north)

isConGrr : isConnectedFun 4 Grr
isConGrr = isConnected-toPullback
...
fibreinr' : Iso (fiber inr' (inl tt)) (Σ (Unit × S₊ 2) PushoutPath×)
fun fibreinr' (x , p) = (tt , x) , (sym p)
inv fibreinr' ((tt , x) , p) = x , (sym p)
rightInv fibreinr' _ = refl
leftInv fibreinr' _ = refl
...
π'P→π'TotalPath× : (n : ℕ) → GroupEquiv (π'Gr n TotalPushoutPath×∙) (π'Gr n P)
fst (fst (π'P→π'TotalPath× n)) =
  π'eqFun n ((invEquiv (isoToEquiv fibreinr')) , refl)
...
module LESinst = πLES {A = S₊∙ 2} {B = thePushout∙} inr∙
...
π₂P≅0 = compGroupEquiv (≃∙→πHom (isoToEquiv fibreinr' , refl) 1)
         (GroupIso→GroupEquiv
  ...
  mapp = connectedTruncIso 4 Grr isConnected-toPullback

  map23 : Iso (π 2 (hLevelTrunc∙ 4 (S₊∙ 3))) (π 2 thePullback∙)
  map23 =
    (compIso (setTruncIso
      (equivToIso (_ ,
...
testComm : (λ x → snd (fst x)) ∘ Grr ≡ fold∘W
testComm = refl
-- nice

-- G → H → L → R
record exact {ℓ ℓ' ℓ'' ℓ''' : Level} (G : Group ℓ) (H : Group ℓ') (L : Group ℓ'') (R : Group ℓ''')
...
P→S²→Pushout :
  exact (πGr 2 P) (πGr 2 (S₊∙ 2)) (πGr 2 thePushout∙) (πGr 1 P)
        (LESinst.fib→A 2)
        (LESinst.A→B 2)
        (LESinst.B→fib 1)
exact.ImG→H⊂KerH→L P→S²→Pushout = LESinst.Im-fib→A⊂Ker-A→B 2
exact.KerH→L⊂ImG→H P→S²→Pushout = LESinst.Ker-A→B⊂Im-fib→A 2
exact.ImH→L⊂KerL→R P→S²→Pushout = LESinst.Im-A→B⊂Ker-B→fib 1
exact.KerL→R⊂ImH→L P→S²→Pushout = LESinst.Ker-B→fib⊂Im-A→B 1

transportExact : {ℓ ℓ' ℓ'' ℓ''' : Level}
...
transportExact G G₂ H H₂ L L₂ R R₂ =
  ...
  where
  abstract
    J4 : ∀ {ℓ ℓ₂ ℓ₃ ℓ₄ ℓ'} {A : Type ℓ} {A₂ : Type ℓ₂} {A₃ : Type ℓ₃} {A₄ : Type ℓ₄}
         {x : A} {x₂ : A₂} {x₃ : A₃} {x₄ : A₄}
         (B : (y : A) (z : A₂) (w : A₃) (u : A₄) → x ≡ y → x₂ ≡ z → x₃ ≡ w → x₄ ≡ u → Type ℓ')
      → B x x₂ x₃ x₄ refl refl refl refl
      → (y : A) (z : A₂) (w : A₃) (u : A₄) (p : x ≡ y) (q : x₂ ≡ z) (r : x₃ ≡ w) (s : x₄ ≡ u)
      → B y z w u p q r s
...
→UnitHom : ∀ {ℓ} (G : Group ℓ) → GroupHom G UnitGr
fst (→UnitHom G) _ = tt
...
test = Σ≡Prop (λ _ → isPropIsGroupHom _ _)
        (funExt (sElim (λ _ → isSetPathImplicit)
          λ f → cong ∣_∣₂ (ΣPathP (refl ,
            (λ j → (λ i → snd (fst ((rUnit (λ k → Grr (snd f k)) (~ j)) i)))
                  ∙ (λ _ → north))))))
...
P→S²→Pushout→Unit =
  transport (λ i → 
    exact (π'Gr 2 P) (π'Gr 2 (S₊∙ 2)) (π'Gr 2 thePushout∙)
          ...
          P→S²→Pushout→P'
  where
  r : PathP (λ i → GroupHom (π'Gr 2 thePushout∙)
                     ((GroupPath _ _ .fst
            (compGroupEquiv (GroupIso→GroupEquiv (π'Gr≅πGr 1 P)) π₂P≅0) i)))
            π₂thePushout→π₁P
            (→UnitHom (π'Gr 2 thePushout∙))
...
extendExactSurj : {ℓ ℓ' ℓ'' ℓ''' ℓ'''' : Level}
  (G : Group ℓ) (H : Group ℓ') (L : Group ℓ'') (R : Group ℓ''') (S : Group ℓ'''')
  (G→H : GroupHom G H) (H→L : GroupHom H L) (L→R : GroupHom L R) (R→S : GroupHom R S)
  → isSurjective G→H
  → exact H L R S H→L L→R R→S
  → exact G L R S (compGroupHom G→H H→L) L→R R→S
ImG→H⊂KerH→L (extendExactSurj G H L R S G→H H→L L→R R→S surj ex) x =
  pRec (GroupStr.is-set (snd R) _ _)
    (uncurry λ g → J (λ x _ → isInKer L→R x)
      (ImG→H⊂KerH→L ex (fst H→L (fst G→H g))
        ∣ (fst G→H g) , refl ∣))
KerH→L⊂ImG→H (extendExactSurj G H L R S G→H H→L L→R R→S surj ex) x ker =
  pRec squash
...
ImH→L⊂KerL→R (extendExactSurj G H L R S G→H H→L L→R R→S surj ex) =
  ImH→L⊂KerL→R ex
KerL→R⊂ImH→L (extendExactSurj G H L R S G→H H→L L→R R→S surj ex) =
  KerL→R⊂ImH→L ex

P'→S²→Pushout→Unit' :
  exact (π'Gr 2 TotalPushoutPath×∙) (π'Gr 2 (S₊∙ 2)) (π'Gr 2 thePushout∙) UnitGr
        (compGroupHom (π'∘∙Hom 2 TotalPushoutPath×∙→P) (π'∘∙Hom 2 (fst , refl)))
        (π'∘∙Hom 2 inr∙)
        (→UnitHom (π'Gr 2 thePushout∙))
P'→S²→Pushout→Unit' =
  extendExactSurj _ _ _ _ _ _ _ _ _
    (sElim (λ _ → isProp→isSet squash)
...
S³→S²→Pushout→Unit =
  subst
   (λ F → exact (π'Gr 2 (S₊∙ 3)) (π'Gr 2 (S₊∙ 2)) (π'Gr 2 thePushout∙) UnitGr
            F (π'∘∙Hom 2 inr∙)
            (→UnitHom (π'Gr 2 thePushout∙)))
            tripleComp≡
   S³→S²→Pushout→Unit''
...
module exact→BoolIso (G : Group₀) (F : GroupHom ℤ ℤ) (H : GroupHom ℤ G) (P : fst F 1 ≡ 2)
         (ex : exact ℤ ℤ G UnitGr F H (→UnitHom G)) where
  ...
  isSurjectiveH b = exact.KerL→R⊂ImH→L ex b refl

  Bool→GHom : IsGroupHom (snd BoolGr) Bool→G strG
  Bool→GHom =
    makeIsGroupHom
      λ { false false → sym (exact.ImG→H⊂KerH→L ex 2 ∣ 1 , P ∣)
  ...
  isSurjectiveBoolGHomPos (suc (suc n)) =
    ...
    ∙ cong (_·G (fst H (pos n)))
        (sym (sym (exact.ImG→H⊂KerH→L ex 2 ∣ 1 , P ∣)))
    ∙ lid (snd G) _
...
exact→Bool≅G± : (G : Group₀) (F : GroupHom ℤ ℤ) (H : GroupHom ℤ G)
         ...
         (ex : exact ℤ ℤ G UnitGr F H (→UnitHom G))
         → GroupIso BoolGr G
exact→Bool≅G± G F H (inl x) ex = exact→BoolIso.Bool≅G G F H x ex
exact→Bool≅G± G F H (inr x) ex =
  exact→BoolIso.Bool≅G _ _ _ (cong (fst flip') x) exact'
  where
  flip' : GroupHom ℤ ℤ
  ...
  exact' : exact ℤ ℤ G UnitGr (compGroupHom F flip') H (→UnitHom G)
  ImG→H⊂KerH→L exact' x inim =
  ...
  ImH→L⊂KerL→R exact' = ImH→L⊂KerL→R ex
...
exact→boolIsoGenPre G H L =
  GroupEquivJ (λ G Z≅G
   ...
   (GroupEquivJ (λ H Z≅H → 
      ...
      exact ℤ H L UnitGr G→H H→L (→UnitHom L) → GroupIso BoolGr L)
      λ Z→Z L P ex → exact→Bool≅G± _ _ L P ex)
...
π₃PushoutCharac :
  ...
  → GroupIso BoolGr (π'Gr 2 thePushout∙)
π₃PushoutCharac p =
  exact→boolIsoGenPre (π'Gr 2 (S₊∙ 3)) (π'Gr 2 (S₊∙ 2)) (π'Gr 2 thePushout∙)
