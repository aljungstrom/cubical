...
-join : (join S¹ S¹ , inl base) →∙ A
     → (join S¹ S¹ , inl base) →∙ A
fst (-join {A = A} f) (inl x) = pt A
fst (-join {A = A} f) (inr x) = pt A
fst (-join {A = A} f) (push a b i) = {!sym (snd f) ∙∙ cong (fst f) (push a b ∙ sym (push base b)) ∙∙ snd f!}
snd (-join f) = refl
...
module _ (A : Pointed ℓ) (con : (isConnected 3 (typ A))) where
  ...
  π₃* = InducedGroup
          ...
          ∣ (λ _ → pt A) , refl ∣₂
          {!!}
          (isoToEquiv π₃*Iso)  -- sElim2 (λ _ _ → isSetPathImplicit) (λ f g → cong ∣_∣₂ (+join≡∙Π f g)))
          {!!}
          {!!}
          {!!}
  {- InducedGroup (π'Gr 2 A) (sRec2 squash₂ (λ x y → ∣ x +join y ∣₂))
        (isoToEquiv π₃*Iso )
