...
asd : (n : ℕ) (g : (Ω (Kℤ/2∙ (suc n)) →∙ Ω (Kℤ/2∙ (n +' (suc n)))))
  → (isProp (fiber Ω→ g))
   × ((x : _) → isProp (Σ[ y ∈ _ ] (pre-alt-fibΩ x y g)))
asd n g = isOfHLevel-fiberΩ→ n 1 g (isConnectedEM (suc n))
  ...
  lem' = cong (suc ∘ suc) (+'≡+ n (suc n)
                        ∙ +-suc n n
                        ∙ +-comm 1 (n + n))
  H : (y : Ω (Kℤ/2∙ (suc n)) →∙ Ω (Kℤ/2∙ (n +' suc n)))
      → isProp (fiber Ω→ y)
  H y = isOfHLevel-fiberΩ→ n 1 y (isConnectedEM (suc n))
         (subst (λ m → isOfHLevel m (Kℤ/2 (n +' suc n)))
           lem'
           (hLevelEM (Ring→AbGroup ℤ/2Ring) (n +' suc n)))
...
snd (deloopKℤ/2FunIso zero i p) = ⊥.rec (snotz (+-comm (suc i) (fst p) ∙ snd p))
snd (deloopKℤ/2FunIso (suc n) i (x , p)) = record { equiv-proof = gr }
  where
  ...
  abstract
    gr : (g : Ω (Kℤ/2∙ (suc (suc n))) →∙ Ω (Kℤ/2∙ (i +' suc (suc n))))
      → isContr (fiber Ω→ g)
    gr g =
...
eq3 : (n i : ℕ) → (i < n)
  → (Kℤ/2∙ (suc n) →∙ Kℤ/2∙ (i +' (suc n)))
   ≃ (Kℤ/2∙ n →∙ Kℤ/2∙ (i +' n))
eq3 n i p = compEquiv (deloopKℤ/2FunIso n i p) (eq2 n i)
...
fst (Sqₖ∙-gen (suc n) (suc i) (lt (zero , p))) x =
  subst (λ m → Kℤ/2 (m +' (suc n))) (sym (cong predℕ p)) (fib-deloop n .fst .fst x)
snd (Sqₖ∙-gen (suc n) (suc i) (lt (zero , p))) =
    cong (subst (λ m → Kℤ/2 (m +' (suc n))) (sym (cong predℕ p))) (fib-deloop n .fst .snd)
  ∙ substEM0ₖ _
...
fst (Sqₖ∙-gen (suc n) (suc i) (eq q)) x =
  subst (λ m → Kℤ/2 (m +' suc n)) (sym q) (cupℤ/2 (suc n) (suc n) x x)
snd (Sqₖ∙-gen (suc n) (suc i) (eq q)) =
    cong (subst (λ m → Kℤ/2 (m +' suc n)) (sym q)) (0ₖ-⌣ₖ (suc n) (suc n) (0ₖ (suc n)))
  ∙ substEM0ₖ _
...
Ω-Sq' (suc n) (suc i) (eq p) =
  ...
  Ω→H≡  : (x : _) → Ω→ H .fst x ≡ subst (fst ∘ Ω ∘ Kℤ/2∙) (λ i → ((sym p i) +' suc (suc n))) x
...
Ω-Sq' (suc n) (suc i) (gt (zero , p)) =
    →∙Homogeneous≡ (isHomogeneousPath _ _)
      (funExt (λ q → cong (substΩEM (+'-suc' (suc i) (suc n)) .fst)
                       (cong (EM→ΩEM+1 (suc (suc (i + n))))
                         (λ j → Sqₖ∙-gen (suc n) (suc i)
