...
module _ {ℓ : Level} {A : Pointed ℓ} (α β : typ ((Ω^ 3) A)) where
  ...
  l-lem : (p : fst (Ω (Ω A))) → EH-alt 0 p refl ≡ sym (rUnit p) ∙ lUnit p
  l-lem p = EH-alt-α-refl 0 p

  r-lem : (p : fst (Ω (Ω A))) → EH-alt 0 refl p ≡ sym (lUnit p) ∙ rUnit p
  r-lem = EH-alt-refl-β 0
  ...
  btm≡₂ : btm-transp1
        ≡ transport (λ i → cong (λ x → (sym (rUnit x) ∙ lUnit x) i) α
             ...
             (λ i → (λ j → α (j ∧ ~ i) ∙ β (j ∧ i)) ∙ λ j → α (~ i ∨ j) ∙ β (i ∨ j))
  btm≡₂ k = transport (λ i → cong (λ x → l-lem x k i) α ∙ cong (λ x → r-lem x k i) β
                    ≡ cong (λ x → r-lem x k i) β ∙ cong (λ x → l-lem x k i) α)
              (λ i → (λ j → α (j ∧ ~ i) ∙ β (j ∧ i)) ∙ λ j → α (~ i ∨ j) ∙ β (i ∨ j))
  F : _ → _
...
-- cong₂Flip : {!∀ {ℓ} {A : Type ℓ} → ? → ? → ?!}
-- cong₂Flip = {!!}

-- syll₂ : ∀ {ℓ} {A : Pointed ℓ} (α β : typ ((Ω^ 3) A))
--      → EH-alt 0 α β
--      ≡ sym (EH-alt 0 β α)
-- syll₂ {A = A} α β k i j =
-- {-
-- k = i0 ⊢ EH-alt 0 (λ i₁ i₂ i₃ → α i₁ i₂ i₃)
--          (λ i₁ i₂ i₃ → β i₁ i₂ i₃) i j
-- k = i1 ⊢ sym
--          (EH-alt 0 (λ i₁ i₂ i₃ → β i₁ i₂ i₃) (λ i₁ i₂ i₃ → α i₁ i₂ i₃)) i j
-- i = i0 ⊢ ((λ i₁ i₂ i₃ → α i₁ i₂ i₃) ∙ (λ i₁ i₂ i₃ → β i₁ i₂ i₃)) j
-- i = i1 ⊢ ((λ i₁ i₂ i₃ → β i₁ i₂ i₃) ∙ (λ i₁ i₂ i₃ → α i₁ i₂ i₃)) j
-- j = i0 ⊢ snd
--          ((Ω^ suc 0)
--           ((snd ((Ω^ 0) A) ≡ snd ((Ω^ 0) A)) , (λ _ → snd ((Ω^ 0) A))))
-- j = i1 ⊢ snd
--          ((Ω^ suc 0)
--           ((snd ((Ω^ 0) A) ≡ snd ((Ω^ 0) A)) , (λ _ → snd ((Ω^ 0) A))))
-- -}
--   hcomp (λ r → λ { (i = i0) → l r j k -- l r j k
--                   ; (i = i1) → {!!}
--                   ; (j = i0) → rUnit refl (~ r)
--                   ; (j = i1) → rUnit refl (~ r)
--                   ; (k = i0) → EH-alt-filler 1 α β i j r
--                   ; (k = i1) → EH-alt-filler 1 β α (~ i) j r})
--         {!!}
--   where

--   p : EH-alt 0 (snd (Ω (Ω A))) (snd (Ω (Ω A))) ≡ refl
--   p z i j = hcomp (λ k → λ {(j = i0) → rUnit refl (~ k)
--                            ; (j = i1) → rUnit refl (~ k)
--                            ; (i = i0) → ((cong (λ x → rUnit x (~ k)) refl) ∙ cong (λ x → lUnit x (~ k)) refl) j
--                            ; (i = i1) → ((cong (λ x → rUnit x (~ k)) refl) ∙ cong (λ x → lUnit x (~ k)) refl) j
--                            ; (z = i1) → ((cong (λ x → rUnit x (~ k)) refl) ∙ cong (λ x → lUnit x (~ k)) refl) j
--                            ; (z = i0) → EH-alt-filler 0 refl refl i j k})
--                 (((cong (λ x → x ∙ refl) refl) ∙ cong (λ x → refl ∙ x) refl) j)

--   s : _
--   s i j =
--     hcomp (λ k → λ { (i = i0) → p k j
--                     ; (i = i1) → p k j
--                     ; (j = i0) → (cong (λ x → x ∙ refl) α ∙ cong (λ x → refl ∙ x) β) i
--                     ; (j = i1) → (cong (λ x → refl ∙ x) α ∙ cong (λ x → x ∙ refl) β) i})
--           ((cong (λ x → EH-alt 0 x refl j) α ∙ cong (λ x → EH-alt 0 refl x j) β) i)

--   l : Cube s (λ j k → (α ∙ β) j)
--            (λ r k → rUnit refl (~ r))
--            (λ r k → rUnit refl (~ r))
--            (λ r j → ((cong (λ x → rUnit x (~ r)) α) ∙ cong (λ x → lUnit x (~ r)) β) j)
--            λ r j → ((cong (λ x → lUnit x (~ r)) α) ∙ cong (λ x → rUnit x (~ r)) β) j
--   l r j k =
--     hcomp (λ w → λ { (r = i1) → {!!}
--                     ; (j = i0) → {!p r k!}
--                     ; (j = i1) → {!p r k!}
...
-- j = i1 ⊢ rUnit refl (~ r)
-- k = i0 ⊢ (cong (λ x → rUnit x (~ r)) α ∙
--           cong (λ x → lUnit x (~ r)) β)
--          j
-- k = i1 ⊢ (cong (λ x → lUnit x (~ r)) α ∙
--           cong (λ x → rUnit x (~ r)) β)
--          j
-- -}

--   ll : EH-alt {A = A} 0 refl refl ≡ refl
--   ll = {!!}

--   w : (λ i → α i ∙ refl) ∙ (λ i → refl ∙ β i) ≡
--       (λ i → refl ∙ β i) ∙ (λ i → α i ∙ refl)
--   w = λ i → (λ j → α (j ∧ ~ i) ∙ β (j ∧ i)) ∙ λ j → α (~ i ∨ j) ∙ β (i ∨ j)

--   ss : PathP (λ k →  (λ j → EH-alt 0 (α j) refl k) ∙ (λ j → EH-alt 0 refl (β j) k)
--                     ≡ (λ j → EH-alt 0 refl (β j) k) ∙ λ j → EH-alt 0 (α j) refl k)
--              w
--              λ i → (λ j → β (j ∧ i) ∙ α (j ∧ ~ i)) ∙ λ j → β (i ∨ j) ∙ α (~ i ∨ j)
--   ss k i = (λ j → EH-alt 0 (α (j ∧ ~ i)) (β (j ∧ i)) k)
--           ∙ λ j → EH-alt 0 (α (~ i ∨ j)) (β (i ∨ j)) k

--   t : cong (λ x → EH-alt 0 x refl) α ≡ {!cong (λ x → lUnit x ∙ sym (rUnit x)) α!}
--   t = {!!}
...
-- {-
-- k = i0 ⊢ EH-alt 0 (λ i₁ i₂ → α i₁ i₂) (λ i₁ i₂ → β i₁ i₂) i
-- k = i1 ⊢ sym (EH-alt 0 (λ i₁ i₂ → β i₁ i₂) (λ i₁ i₂ → α i₁ i₂)) i
-- i = i0 ⊢ (λ i₁ i₂ → α i₁ i₂) ∙ (λ i₁ i₂ → β i₁ i₂)
-- i = i1 ⊢ (λ i₁ i₂ → β i₁ i₂) ∙ (λ i₁ i₂ → α i₁ i₂)
-- -}


-- -- s : {ℓ : Level} {A : Type ℓ} {x y : A} {B : {x y : A} → x ≡ y → x ≡ y → Type ℓ}
-- --       (comp : (p q : x ≡ y) → B p q)
-- --    → {!!}
-- -- s = {!!}
